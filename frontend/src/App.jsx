...
function App() {
  ...
  useEffect(() => {
    if (activeTab === 'scan' && streamMode === 'local') {
      setCameraSrc(`${API_BASE}/api/video_feed?t=${Date.now()}`);
    } else {
      setCameraSrc('');
    }
  }, [activeTab, streamMode]);
  ...
  useEffect(() => {
    ...
    if (activeTab === 'scan' && streamMode === 'cloud') {
      let isStopped = false;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch flex-1">
            ...
            <div className="lg:col-span-7 flex flex-col">
              <div className="glass-panel p-6 flex-1 flex flex-col items-center justify-between relative overflow-hidden min-h-[480px]">
                ...
                <div className="z-10 flex gap-3 mb-4 w-full justify-center">
                  <button 
                ...
                {streamMode === 'local' ? (
                  <div className="w-full max-w-2xl rounded-xl border border-white/10 overflow-hidden bg-black shadow-inner flex items-center justify-center relative">
                    {cameraSrc ? (
                      <img 
                        src={cameraSrc} 
                        alt="Camera feed"
                ...
                <div className="mt-4 flex items-center gap-3 bg-white/5 px-5 py-2.5 rounded-full border border-white/5">
                  <div className={`w-2.5 h-2.5 rounded-full animate-pulse ${streamMode === 'local' ? 'bg-cyan-400' : 'bg-indigo-400'}`}></div>
                  <span className={`text-xs font-semibold tracking-wider ${streamMode === 'local' ? 'text-cyan-300' : 'text-indigo-300'}`}>
                    {streamMode === 'local' 
